...
hgeGUIMenuItem::hgeGUIMenuItem(int _id, hgeFont *_fnt, HEFFECT _snd, float _x, float _y, float _delay, char *_title)
{
	...
	offset=0.0f;
	timer=-1.0f; // something to do with colors and offsets
	timer2=-1.0f; // seems to have something to do with colors and shadows.
...
void hgeGUIMenuItem::Focus(bool bFocused)
{
	...
	{
		hge->Effect_Play(snd);
		scolor.SetHWColor(0xFFFFE060); // s turn orangeish
		tcolor.SetHWColor(0xFFFFFFFF); // t turn whiteish
		soffset=0;
		doffset=4;
	}
	else
	{
		scolor.SetHWColor(0xFFFFFFFF); // s turn white,
		tcolor.SetHWColor(0xFFFFE060); // t turn orangeish
		soffset=4;
		doffset=-4;
	}
